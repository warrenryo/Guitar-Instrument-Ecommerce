<div>
    ...
    @if($products->isEmpty())
    <h1 class="block items-center justify-center text-lg text-semibold mb-[34.5vh] mt-[20vh]">No Instrument found</h1>
    @else
    <div class="mx-auto mb-10 grid max-w-[141vh] lg:max-w-full xl:max-w-full grid-cols-1 gap-6 p-6 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 1366:grid-cols-5">
        ...
        <article class="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
            <a href="{{ url('products/'.$product->slug) }}">
                ...
                <div class="mt-1 p-2">
                    ...
                    <p class="text-md text-red-500"><span class="bg-red-500 py-0 px-2 rounded-sm text-white ">Sale!</span> <i class="fa-solid fa-peso-sign"></i> <span id="price">{{number_format($product->sale_price)}} </span><span class="text-gray-400 text-sm italic ml-2 line-through"><i class="fa-solid fa-peso-sign"></i><span id="price">{{ number_format($product->original_price)}}</span></span></p>
                        @else
                        <p class="text-md text-blue-500"><i class="fa-solid fa-peso-sign"></i> <span id="price">{{ number_format($product->original_price)}}</span></p>
                    @endif
                    </div>
                </div>
            </a>
