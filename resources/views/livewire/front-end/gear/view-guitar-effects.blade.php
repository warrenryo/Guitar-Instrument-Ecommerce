...
<div class="mx-auto mb-10 grid max-w-[141vh] lg:max-w-full xl:max-w-full  grid-cols-1 gap-6 p-6 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-5 1366:grid-cols-5">
        @forelse($guitarEffectsProd as $gearProduct)
        <article class="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 "
            <a href="{{ url('guitar-effects/'.$gearProduct->slug) }}">
                <div class="relative flex items-center justify-center overflow-hidden mt-2">
                    <div>
                        <img src="{{ asset($gearProduct->guitarEffectsImages[0]->images) }}" alt="Hotel Photo" class="h-[300px] rounded-xl" />
                ...
                <div class="mt-1 p-2">
                        <h2 class="text-sm tracking-tight text-gray-900 dark:text-white h-[60px]"><span class="text-sm">{{$gearProduct->brand}}</span> {{$gearProduct->guitar_effects_name}}</h2>

                    <div class="mt-3 flex items-end justify-between">
                    @if($gearProduct->sale_price < $gearProduct->orig_price)
                    <p class="text-md text-red-500"><span class="bg-red-500 py-0 px-2 rounded-sm text-white ">Sale!</span> <i class="fa-solid fa-peso-sign"></i> {{number_format($gearProduct->sale_price)}} <span class="text-gray-400 text-sm italic ml-2 line-through"><i class="fa-solid fa-peso-sign"></i>{{$gearProduct->orig_price}}</span></p>
                        @else
