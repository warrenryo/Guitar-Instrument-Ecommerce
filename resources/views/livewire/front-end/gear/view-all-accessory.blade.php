...
<div class="mx-auto mb-10 grid max-w-[141vh] lg:max-w-full xl:max-w-full grid-cols-1 gap-6 p-6 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-5 1366:grid-cols-5">
        @forelse($allAccessory as $gearProduct)
        <article class="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
            <a href="{{ url('accessories/'.$gearProduct->slug) }}">
                <div class="relative flex items-center justify-center overflow-hidden mt-2">
                    <div>
                        <img src="{{ asset($gearProduct->acessoryImages[0]->images) }}" alt="Hotel Photo" class="h-[300px] rounded-xl" />
                ...
                <div class="mt-1 p-2">
                        <h2 class="text-sm tracking-tight text-gray-900 dark:text-white h-[80px]"><span class="text-sm">{{$gearProduct->brand}}</span> {{$gearProduct->accessory_name}} <span>{{$gearProduct->gearCategory->accessory_category_name}}</span></h2>
