...
<div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-10 text-gray-900">
                    <div class="mt-4">
                        <h1 class="font-bold text-2xl">Thank you for your order!</h1>
                    </div>
                    <div class="flex justify-between mt-4">
                        <div class="block">
                            <h1 class="text-lg font-semibold ">Hi <span class="text-gray-500">{{$orderSummary->firstName}} {{$orderSummary->lastName}}</span> !</h1>
                            <p class="text-sm">Your order has been placed and will be ready soon</p>
                        </div> 
                        <div class="block">
                            <p class="text-lg font-semibold">Ordered Date</p>
                            <h1 class="text-sm text-gray-500">{{$orderSummary->created_at->format('d-m-y H:i:s')}}</h1>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-10">
                        <div class="block mt-10">
                            ...
                            <p class="text-gray-500 mt-4 text-sm"># {{$orderSummary->tracking_number}}</p>

                        </div>
                        
                        <div class="block mt-10">
                            <h1 class="text-lg font-semibold ">Contact Info</h1>
                            <p class="text-gray-500 mt-4 text-sm"><i class="fa-solid fa-phone"></i> {{$orderSummary->phoneNumber}}</p>
                            <p class="text-gray-500 mt-4 text-sm"><i class="fa-solid fa-envelope"></i> {{$orderSummary->contactInfo}}</p>
                        </div>
                        <div class="block mt-10">
                            <h1 class="text-lg font-semibold ">Billing Address</h1>
                            <p class="text-gray-500 mt-4 text-sm">  {{$orderSummary->company}} {{$orderSummary->address}} {{$orderSummary->apartment}} {{$orderSummary->postalCode}} {{$orderSummary->city}}</p>
                            
                        </div>
                        <div class="block mt-10">
                            <h1 class="text-lg font-semibold ">Order Information</h1>
                            <p class="text-gray-500 mt-4 text-sm uppercase">{{$orderSummary->payment_method}}</p>
                            @if($orderSummary->delivery_method == 'ship')
                    ...
                    <div class="sm:block lg:flex">
                        <div class="block">
                        @foreach($orderSummary->cartOrders as $item)
                            @if($item->product)                   
                    ...
                    <div class="py-6 mt-4">
                        <p class="text-gray-500 text-sm">We will send you shipping information on your Email "{{$orderSummary->contactInfo}}" when your items are on the way! We appreciate your business, and hope you enjoy your purchase Thank you!</p>
                    </div>
    ...
    <script>
        @if($orderSummary->status == 'Order Placed')
            ...
            @elseif($orderSummary->status == 'Processing')
            const orderPlaced = document.getElementById('status1');
            ...
            @elseif($orderSummary->status == 'Shipped')
            const orderPlaced = document.getElementById('status1');
            const orderPlaced2 = document.getElementById('status2');
            ...
            @elseif($orderSummary->status == 'Delivered')
            const orderPlaced = document.getElementById('status1');
            const orderPlaced2 = document.getElementById('status2');
            const orderPlaced3 = document.getElementById('status3');
