<title>Check Out - {{$buyOneGuitarEffects->guitar_effects_name}}</title>
<x-app-layout>
...
<form action="{{ url('one-order') }}" method="POST" id="formRequest">
    @csrf
        <div class="block">
            ...
            <input type="text" class="hidden" name="product_slug" value="{{$buyOneGuitarEffects->slug}}">
    ...
    <div class="block ml-10 w-[50vh] sm:pt-20 xl:pt-0 sm:mb-20 xl:mb-0">
        <img src="{{ asset($buyOneGuitarEffects->guitarEffectsImages[0]->images) }}" alt="" class="w-[70vh] h-[75vh]">
        <h1 class="text-2xl tracking-tight text-gray-900 dark:text-white mt-10 "><span class="text-2xxl">{{$buyOneGuitarEffects->brand}}</span> {{$buyOneGuitarEffects->guitar_effects_name}} <span>{{$buyOneGuitarEffects->guitarEffectsCategory}}</span></h1>
        <div class="mt-10 ">
            <div class="flex">
            ...
            <div class="flex">
                ...
                <div class="mt-2 flex-grow text-right ">
                    @if($buyOneGuitarEffects->sale_price < $buyOneGuitarEffects->orig_price)
                    <h1 >{{number_format($buyOneGuitarEffects->sale_price)}}</h1>
                        @else
                        <h1><i class="fa-solid fa-peso-sign"></i> {{number_format($buyOneGuitarEffects->orig_price)}}</h1>
                    @endif
                </div>
            </div>
            <div class="flex">
            ...
            <div class="flex">
                ...
                <div id="h1Element" class="mt-2 flex-grow text-right font-bold">
                    ...
                    <h1>{{number_format($buyOneGuitarEffects->sale_price)}}</h1>
                        @else
                        @php $total += $buyOneGuitarEffects->orig_price * $quantity @endphp
