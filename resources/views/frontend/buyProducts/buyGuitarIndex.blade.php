<title>Check Out - {{$guitarProducts->product_name}}</title>
<x-app-layout>
...
<form action="{{ url('orders') }}" method="POST">
    @csrf
        <div class="block">
            ...
            <input type="hidden" name="productId" value="{{$guitarProducts->id}}">
            <input type="hidden" name="productCategory" value="{{$guitarProducts->category->name}}">
            <input type="hidden" name="productBrand" value="{{$guitarProducts->brand}}">
            <input type="hidden" name="productName" value="{{$guitarProducts->product_name}}">
            <input type="hidden" name="productColor" value="{{$guitarProducts->color}}">
    ...
    <div class="block ml-10 w-[50vh]">
        <img src="{{ asset($guitarProducts->productImages[0]->images) }}" alt="" class="w-[50vh] h-[75vh]">
        <h1 class="text-2xl tracking-tight text-gray-900 dark:text-white mt-10 "><span class="text-2xxl">{{$guitarProducts->brand}}</span> {{$guitarProducts->product_name}} <span>{{$guitarProducts->category->name}}</span> - <span>{{$guitarProducts->color}}</span></h1>
        <div class="mt-10 ">
            <div class="flex">
                <h2 class="mt-2">SubTotal</h2>
                <div class="mt-2 flex-grow text-right ">
                    @if($guitarProducts->sale_price < $guitarProducts->original_price)
                    <h1 >{{number_format($guitarProducts->sale_price)}}</h1>
                        @else
                        <h1><i class="fa-solid fa-peso-sign"></i> {{number_format($guitarProducts->original_price)}}</h1>
                    @endif
                </div>
            </div>
            <div class="flex">
